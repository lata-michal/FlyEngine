...
feng::FrameBuffer::FrameBuffer(const VBO& vbo, const IBO& ibo, VertexLayout& layout)
...
void feng::FrameBuffer::Draw(const Shader& postProcessing)
{
	if (Window::QueryAntiAliasing())
	{
...
void feng::FrameBuffer::Update()
{
	if (Window::QueryAntiAliasing())
	{
		...
		glBindFramebuffer(GL_FRAMEBUFFER, m_FrameBuffer);
		glBindTexture(GL_TEXTURE_2D, m_TexColorBuffer);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, Window::GetWidth(), Window::GetHeight(), 0, GL_RGBA, GL_HALF_FLOAT, nullptr);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
...
void feng::FrameBuffer::Bind() const
{
	if (Window::QueryAntiAliasing())
		glBindFramebuffer(GL_FRAMEBUFFER, m_MultiSampleFrameBuffer);
	else
...
float feng::FrameBuffer::GetExposure(float exposureFactor, float minimumExposure, float maximumExposure, float exposureAdjustmentSpeed)
{
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, m_TexColorBuffer);
	glGenerateMipmap(GL_TEXTURE_2D);
	glGetTexImage(GL_TEXTURE_2D, static_cast<uint32_t>(log2(static_cast<float>(max(Window::GetWidth(), Window::GetHeight())))), GL_RGB, GL_HALF_FLOAT, &m_SceneColor);
...
void feng::FrameBuffer::Unbind() const { glBindFramebuffer(GL_FRAMEBUFFER, 0); }

feng::FrameBuffer::~FrameBuffer() { glDeleteFramebuffers(1, &m_FrameBuffer); glDeleteRenderbuffers(1, &m_RenderBuffer); }