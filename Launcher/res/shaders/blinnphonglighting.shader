...
void main()
{
    ...
    for(int i = 0; i < 4; i++)
        result += calcPointLight(uPointLight[i], norm, oFragPos, viewDir);

    //result += calcSpotLight(uSpotLight, norm, oFragPos, viewDir);
...
vec3 calcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
    ...
    vec3 diffuse = light.diffuse * diff * vec3(texture(uMaterial.texture_diffuse1, oTexCoord));

    vec3 specular = light.specular * spec * vec3(texture(uMaterial.texture_specular1, oTexCoord));
