...
vec2 parallaxMapping(vec2 texCoordinate, vec3 viewDir)
{
    ...
    float currentDispMapValue = texture(uMaterial.texture_displacement1, currentTexCord).r;

    while(currentDepth < currentDispMapValue)
    {
        currentDepth += layerDepth;
        currentTexCord -= deltaTexCoord;
        currentDispMapValue = texture(uMaterial.texture_displacement1, currentTexCord).r;
    }

    vec2 prevTexCoord = currentTexCord + deltaTexCoord;

    float beforeDepth = texture(uMaterial.texture_displacement1, prevTexCoord).r - currentDepth + layerDepth;
